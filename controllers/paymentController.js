...
//   let { session_id, loan_id, emi_amount } = req.query;
//   console.log("emi_amount", emi_amount);

//   if (!session_id || !loan_id) {
//     return res.status(400).json({ error: "Missing session_id or loan_id" });
//   }
...
exports.retryPayment = async (req, res) => {
    ...
    try {
      ...
      const newSession = await stripe.checkout.sessions.create({
        ...
        mode: "payment",
        success_url: `http://localhost:3000/payment/success?loan_id=${loan_id}`,
        cancel_url: `http://localhost:3000/payment/cancel?loan_id=${loan_id}&emiAmount=${emi_amount}`,
